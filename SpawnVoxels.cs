...
/// <summary>
/// This is the mono version of voxelizing class. It uses Raycast method to detect edges of the Object_To_Voxelize.
...
public class SpawnVoxels : MonoBehaviour {
	...
	private int x_voxels, y_voxels, z_voxels, nr_voxels_created = 0, nr_mesh_triangles;
	...
	{		
		...
		{
			Debug.LogError("Voxelizer: No voxel prefab has been assigned.");
			return;
		}
		if (this.transform.GetComponent<MeshRenderer>() == null || this.transform.GetComponent<MeshRenderer>().enabled == false)
		{
			Debug.LogError("Voxelizer: Object to voxelize has no active MeshRenderer.");
			return;
		}
		if (this.transform.GetComponent<MeshFilter>() == null)
		{
			Debug.LogError("Voxelizer: Object to voxelize has no MeshFilter.");
			return;
		}
		if (this.transform.GetComponent<MeshFilter>() != null && this.transform.GetComponent<MeshFilter>().sharedMesh == null)
		{
			Debug.LogError("Voxelizer: MeshFilter component of object to voxelize has no Mesh assigned.");
			return;
		}
		if (Voxel_Material.Material_From_Object == true && (this.transform.GetComponent<MeshCollider>() == null || this.transform.GetComponent<MeshCollider>().enabled == false))
		{
			Debug.LogWarning("Voxelizer: Material_From_Object won't work because it requires active MeshCollider.");
		}
		if (this.transform.GetComponent<Collider>() == null || this.transform.GetComponent<Collider>().enabled == false)
		{
			Debug.LogError("Voxelizer: Object to voxelize hasn't got any active Collider.");
			return;
		}
		...
		bounds = this.transform.GetComponent<MeshRenderer>().bounds;
		bounds.Expand(1);		
		...
		object_layer = 1<<this.gameObject.layer;
		...
		correction_left = ((this.transform.position.x - this.transform.localScale.x/2) - (bounding_box.transform.position.x - bounding_box.transform.localScale.x/2))/Prefab_Voxel.transform.localScale.x;
		...
		correction_right = ((bounding_box.transform.position.x + bounding_box.transform.localScale.x/2) - (this.transform.position.x + this.transform.localScale.x/2))/Prefab_Voxel.transform.localScale.x;
		...
		correction_down = ((this.transform.position.y - this.transform.localScale.y/2) - (bounding_box.transform.position.y - bounding_box.transform.localScale.y/2))/Prefab_Voxel.transform.localScale.y;
		...
		correction_up = ((bounding_box.transform.position.y + bounding_box.transform.localScale.y/2) - (this.transform.position.y + this.transform.localScale.y/2))/Prefab_Voxel.transform.localScale.y;
		correction_up = correction_up - Mathf.Floor(correction_up);
		correction_up = correction_up * Prefab_Voxel.transform.localScale.y;
		correction_back = ((this.transform.position.z - this.transform.localScale.z/2) - (bounding_box.transform.position.z - bounding_box.transform.localScale.z/2))/Prefab_Voxel.transform.localScale.z;
		...
		voxel_parent = new GameObject(this.gameObject.name + "_voxels");
		foreach (Vector3 pos in voxel_positions)
		{
			...
			nr_voxels_created++;
			voxel.name = this.transform.name + "_voxel_" + nr_voxels_created.ToString();
			...
			if (Voxel_Material.Material_From_Object == true)
			{
				voxel.renderer.material = this.renderer.material;
				...
				voxel.GetComponent<MeshFilter>().mesh.uv = uvs2;
			}
			else
			{
			...
			if (Voxel_Components.Add_Rigidbody == true || Voxel_Joints.CreateJoints == true)
			...
			foreach (KeyValuePair<Vector3, Vector3> p in voxel_pairs)
			{
				for (int i=1; i<Mathf.Round((p.Value.x-p.Key.x)/Prefab_Voxel.transform.localScale.x); i++)
				{
					...
					if (fill_raycast[position_dec] == 6)
					{
						...
						voxel.transform.parent = voxel_parent.transform;
						nr_voxels_created++;
						voxel.name = this.transform.name + "_voxel_fill_" + nr_voxels_created.ToString();
		...
		Destroy(bounding_box);
		if (Info == true)
		{
			Debug.Log("Voxelizer summary\nObject: "+this.gameObject.name+", Number of created voxels: "+nr_voxels_created+", Time: "+(Time.realtimeSinceStartup-time1)+" seconds.");
		}
